...
- 认证安全升级：将当前 Session 登录逐步扩展为 JWT / Redis Session，并将密码加密方式从加盐 MD5 升级为 BCrypt。
- 上传能力增强：当前头像保存到本地目录，后续可接入对象存储，并补充图片压缩、格式校验和旧文件清理。
- 权限模型细化：在现有普通用户 / 管理员角色基础上，扩展为菜单级、按钮级的 RBAC 权限控制。
- 审计日志完善：增加更丰富的筛选条件、批量导出、操作详情展示和高风险操作标记。
- 部署体验优化：补充 Docker Compose，一键启动 MySQL、后端和前端，并完善生产环境配置说明。
- 测试覆盖提升：补充注册、登录、用户管理、封禁解封、头像上传等核心流程的单元测试和接口测试。
