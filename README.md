...
一个前后端分离的用户中心全栈项目，提供注册登录、个人资料、头像上传、用户管理、角色权限、封禁解封和访问审计等能力。

## 项目亮点

- 支持用户注册、登录、退出登录、个人中心和个人设置。
- 注册时可填写用户名、头像、性别、手机号、邮箱等资料。
- 登录逻辑区分用户不存在、密码错误、账号封禁等异常场景。
- 后端统一做用户信息脱敏，避免返回密码、删除标记等敏感字段。
- 管理员可进行用户搜索、创建、编辑、删除、角色调整、封禁和解封。
- 支持头像上传和关键操作审计日志。
